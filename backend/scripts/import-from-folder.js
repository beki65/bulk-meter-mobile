...
async function processFile(filePath, month, year) {
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', async (line) => {
      ...
      // Pause while we process this record
      rl.pause();
      
      try {
        // Check if customer exists
        let customer = await Customer.findOne({ custKey });
        ...
        if (!customer) {
          ...
          // Existing customer - check if this month already exists
          const monthExists = customer.billingHistory.some(
            b => b.month === month && b.year === year
          );
          
          if (!monthExists) {
            customer.billingHistory.push(monthlyBill);
            
            // Update location if missing
            if (!customer.latitude) {
              customer.latitude = x;
              customer.longitude = y;
              customer.location = { type: 'Point', coordinates: [y, x] };
            }
            
            await customer.save();
            updated++;
          }
          // If month exists, skip silently
        }
      } catch (error) {
        if (error.code === 11000) {
          // Duplicate key - just skip, it's fine
          console.log(`   ⚠️  Duplicate key for ${custKey}, skipping...`);
        } else {
          console.error(`   ❌ Error with ${custKey}:`, error.message);
        }
      }
    ...
    rl.on('close', () => {
      resolve({ processed, created, updated, skippedNoCoords, skippedNoKey });
    });
...
async function importAllFiles() {
  console.log('🚀 FAST IMPORT - Processing one month at a time');
  console.log('==============================================');
  ...
  for (const file of files) {
    ...
    console.log(`      New customers: ${result.created.toLocaleString()} | Updated: ${result.updated.toLocaleString()}`);
  ...
  samples.forEach((c, i) => {
    console.log(`   ${i+1}. ${c.custKey}: ${c.name} - (${c.latitude}, ${c.longitude}) - ${c.billingHistory.length} months`);
  });
